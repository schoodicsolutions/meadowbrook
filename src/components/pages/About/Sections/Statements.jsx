    ...
    return (
        <>
            <section className='w-full h-full'>
                <div className='px-4 md:px-12 2xl:px-0 my-12 w-full max-w-[68rem] mx-auto pt-0 md:pt-5 text-white'>
                    <div className='w-full flex flex-col gap-y-8 sm:gap-y-14 md:gap-y-24 flex-1'>
                        {States.map((state, index) => (
                            <div key={state.id} className={(index === 0 ? `${style} flex-row` : '') + ' ' + (index === 1 ? `${style} md:flex-row lg:flex-row-reverse` : '')}>
                                <div className='text-black text-center sm:text-left w-full max-w-[480px] pb-6 lg:pb-0'>
                                    <p className='text-black uppercase text-xs tracking-[0.3em] font-black'>{state.title}</p>
                                    <h1 className='font-black text-[30px] md:text-[36px] 2xl:text-[40px] leading-snug py-2 lg:py-3'>
                                    ...
                                    <p className='text-sm sm:text-base'>{state.detail}</p>
                                </div>
