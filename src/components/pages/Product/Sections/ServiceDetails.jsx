import React, { useEffect } from 'react'
import { Link, useParams } from 'react-router-dom';
import { StumpGrind, TreeRemoval, WallRetain, LandLot } from '../../../../assets';
function ServiceDetails() {

  const { serviceDetail } = useParams(); // Get the serviceName from the URL
  const { serviceName } = useParams(); // Get the serviceName from the URL

  // Replace hyphens with spaces
  const ServiceDetailName = serviceDetail.replace(/-/g, ' ');

  const ServiceMatch = () => {
    if (ServiceDetailName === "stump grinding") {
      return StumpGrind; // Return the image URL for "stump-grinding"
    }
    else if (ServiceDetailName === "tree removal") {
      return TreeRemoval; // Return the image URL for "stump-grinding"
    }
    else if (ServiceDetailName === "land lot clearing") {
      return LandLot; // Return the image URL for "stump-grinding"
    }
    else if (ServiceDetailName === "retaining walls") {
      return WallRetain; // Return the image URL for "stump-grinding"
    }
  }

  // Get the image URL based on the service name
  const serviceImage = ServiceMatch();

  useEffect(() => {
    // Scroll to the top when the page loads
    window.scrollTo(0, 0);
  }, [serviceDetail]);

  return (
    <>
      <section className='bg-[#05011C] relative lg:flex mb-10 flex-wrap lg:flex-nowrap'>
        <div className='px-6 md:px-16 2xl:px-12 pb-8 lg:pb-28 2xl:pb-0 w-full lg:max-w-[47rem] ml-auto pt-5 md:pt-5'>
          {/* BreadCrumbs */}
          <div className='text-white capitalize pt-2'>
            <div className='text-base'>
              <Link className='font-bold' to='/'>Home </Link>/
              <Link className='font-bold' to={`/products/${serviceName}`}> {serviceName}</Link> /
              <Link> {ServiceDetailName}</Link>
            </div>
          </div>
          <div className='text-white h-full mt-8 md:mt-12 2xl:mt-20'>
            <h1 className='leading-normal text-[28px] sm:text-[36px] lg:text-[38px] 2xl:text-[3rem] font-black w-full max-w-md sm:max-w-lg lg:max-w-full '>
              <span className='capitalize'>{ServiceDetailName} Services: </span>
              <span >Unearth the Beauty Beneath</span>
            </h1>
            <p className='text-lg pt-1'>A Service from meadowbrook</p>
          </div>
        </div>
        <div className='relative h-auto'>
          <div className='relative h-auto object-cover'>
            <svg class="absolute lg:block hidden h-full top-0 left-0 -ml-6 fill-scarlet"><path d="M185.261 0H1018V407H0L185.261 0Z"></path></svg>
            <img src={serviceImage} className='relative hidden lg:block h-full object-cover' style={
              {
                clipPath: 'path("M 159.668 0 H 995 V 407 H 0 L 159.668 0 Z")',

              }
            } />
            <img src={serviceImage} className='lg:hidden w-full' />
          </div>
        </div>
      </section>
    </>
  )
}

export default ServiceDetails
